...
| `USES_DOCUMENT` | `uses_document` | Host artifact references a Document (`object_type=="document"`) — a processModel node input (ADR 0034, IV-190; same channel as `uses_ai_skill`; no re-source, host unconstrained, same as `uses_folder`) or a site/portal page's `<uiObject xsi:type="a:CollaborationDocument">` (ADR 0034, IV-192; re-sourced onto the owning `sitePage` node when declared, IV-444 — same `contains_page`-gated re-source `CALLS` above describes for a page). |
